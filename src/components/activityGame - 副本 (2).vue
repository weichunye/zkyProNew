<template>
  <div class="activityGame">
  	...
  	<div class="banner-top">
  		...
  		<div class="reposbox">
  			<router-link  to="/"><div title="返回社区首页"  class="logo">
  		    
  		</div></router-link>
  		<router-link  to="/"><p class="back-index">返回社区首页</p></router-link>
  		...
  		<!--    	2019年5月1日正式开启报名-->
  		    ...
  		    <div v-if="activityInfo.status==2&&ifMobile==false" @click="signUpActity" class="sign-up-btn">
  		    	</div>
  		    <!--	<router-link   :to="{path:'/signUp',query:{activityId:activityInfo.id}}">-->
  		    	 <div v-if="activityInfo.status==2&&ifMobile" @click="signUpActity" class="sign-up-btn">
  		</div>
  		<!--</router-link>-->
  		</div>
  	</div>
...
<el-dialog  custom-class="signdialog " :close-on-click-modal="false" :close-on-press-escape="false"  title="报名信息" :visible.sync="dialogFormVisible">
  <el-form :model="form" ref="form"   :inline="true" class="demo-form-inline" @submit.native.prevent>
    ...
    <el-option
      v-for="item in opensourceTypeOption "
      :key="item.id"
      :label="item.ctyName"
      :value="item">
    </el-option>
    ...
    <el-option
      v-for="item in softCategoryOption "
      :key="item.id"
      :label="item.ctyName"
      :value="item">
    </el-option>
   ...
   <el-select v-model="form.userInterface"  value-key="id" filterable multiple  placeholder="请选择用户接口">
    <el-option
      v-for="item in userInterfaceOption "
     :key="item.id"
      :label="item.ctyName"
      :value="item">
    </el-option>
    ...
    <el-option
      v-for="item in applicationFieldOption "
     :key="item.id"
      :label="item.ctyName"
      :value="item">
    </el-option>
	...
	_this.axios.post(baseUrl.baseUrl+'/web/activity/activityInfo ',params)
  	.then(function(response){
		...
		console.log("_this.activityData",response)
  		...
  		_this.matchStartDate=_this.activityInfo.matchStartDate.substring(0,10)
  		_this.matchEndDate=_this.activityInfo.matchEndDate.substring(0,10)
